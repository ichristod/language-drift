...
def train(args, data, unigram_distribution, word_vectors, dataset_filename, path_to_save, doc_weights_init, n_topics):
    ...
    model_name = "mat1" if dataset_filename == "corpus1" else "mat2"

    start_epoch = 1
    # if checkpoint exists
    model_list = glob(os.path.join(path_to_save, model_name+'.pt'))
    model_list2 = []
    ...
    try:
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            if epoch % save_every == 0:
                ...
                torch.save(model.state_dict(),  str(path_to_save) + '/' + model_name + '.pt')
