...
class TWEC:
    ...
    def train_compass(self, compass_text, path_output, overwrite=False):
        compass_exists = os.path.isfile(get_file_path(path_output)+ "/corpus_concat.model")

        if compass_exists and overwrite is False:
            self.compass = Word2Vec.load(get_file_path(path_output)+ "/corpus_concat.model")
            ...
            self.compass.save(path_output+".model")
            # Save the vectors
            self.compass.wv.save_word2vec_format(path_output)

        self.gvocab = self.compass.wv.vocab

    def train_slice(self, slice_text, path_output, save=True):
        compass_exists = os.path.isfile(get_file_path(path_output)+ "/corpus_concat.model")
        ...
        model_name = os.path.splitext(os.path.basename(path_output))[0]
